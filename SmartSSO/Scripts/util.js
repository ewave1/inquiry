(function (window) {
    ...
    util.checkIgkey = function(v,tl,type){
        var _illegalList = [];
        for(var i = 0;i<tl.length;i++){
            if(v.lastIndexOf(tl[i])>-1){
                if(!String(_illegalList.join(',')).lastIndexOf(tl[i])>-1){
                   _illegalList.push(tl[i]);
                }
            }
        }
        _illegalList.length>0 && util.message(type+'：'+_illegalList.join(','));
        return _illegalList;
    }


    util.illegalList = function(){
        var _illegalList = String('国家级、世界级、最高级、最佳、最大、唯一、首个、首选、最好、精确、顶级、最高、最低、最具、最便宜、最新、最先进、最新技术、最先进科学、国际级产品、填补国内空白、绝对、独家、首家、最先进、第一品牌、金牌、名牌、优秀、最先、顶级、独家、全网销量第一、全球首发、全网首发、世界领先、顶级工艺、最新科学、最新技术、最先进加工工艺、最时尚、极品、顶级、顶尖、终极、最受欢迎、王牌、销量冠军、第一、极致、永久、王牌、掌门人、领袖品牌、独一无二、独家、绝无仅有、前无古人、史无前例、万能、最').split('、');
        return _illegalList;
    }
    util.contrabandList = function(){
        var _illegalList = String('兰蔻、资生堂').split('、');
        return _illegalList;
    }


    util.checkIllegal = function(v){
        util.checkIgkey(v, util.illegalList(), '违禁词');
    }
    util.checkContraband = function(v){
        util.checkIgkey(v, util.contrabandList(), '竞品词');
    }
    ...
    util.message = function (msg, redirect, type) {
        ...
        var content =
			...
			'			<div class="pull-left"><p style="margin:15px 0px">' + msg + '</p>' +
			p +
			'			</div>' +
        ...
        modalobj.find('.modal-content').addClass('alert alert-info' + type);
        if (redirect) {
            var timer = '';
            timeout = 3;
    ...
    util.confirm = function (title, content, callback) {
        var footer = '<button type="button" class="btn btn-default btnOK" >确认</button> <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>';

        var content =
			'<i class="pull-left fa fa-4x fa-info-circle"></i>			\
            <div class="pull-left"><p style="margin:15px 0px">' + content +
            '</p>			</div>			<div class="clearfix"></div>';

        var modalobj = util.dialog(title, content, footer, { containerName: 'icon-container' });
        //modalobj.modal({ 'keyboard': false });
        //modalobj.find('.modal-dialog').css({ 'width': '40%' });
        //modalobj.find('.modal-body').css({ 'height': '40%', 'overflow-y': 'scroll' });
         modalobj.find('.modal-content').addClass('alert alert-info' );
        modalobj.modal('show');

        modalobj.find('.btnOK').on('click', function () {
            if (callback)
                callback();
            modalobj.modal('hide');
        })
    ...
    util.emojiBrowser = function (callback) {
        var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>';
        var modalobj = util.dialog('请选择表情',['/resource/js/app/emoji.html'] , footer, { containerName: 'icon-container' });
        modalobj.modal({ 'keyboard': false });
        modalobj.find('.modal-dialog').css({ 'width': '70%' });
        modalobj.find('.modal-body').css({ 'height': '70%', 'overflow-y': 'scroll' });
        modalobj.modal('show');
